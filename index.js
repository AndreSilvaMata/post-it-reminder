    ...
    const taskRender = function (task) {
        ...
        task_date.innerText = new Date().toISOString().slice(0, 10);

        task_actions_el.appendChild(task_date);

        // eventos click dos botoes edit check delete date
        task_check_el.addEventListener('click', (e) => {
            task_input_el.classList.toggle('check-task');
        });
